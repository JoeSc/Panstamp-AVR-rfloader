...
class SWAP
{
  public:
    ...
    void init(uint8_t freq=CFREQ_868)
    {
      ...
      nvMem.read(buf, DEFAULT_NVOLAT_SECTION, NVOLAT_SIGNATURE, 2);

        nvMem.read(buf, DEFAULT_NVOLAT_SECTION, NVOLAT_FREQ_CHANNEL, 1);
        printf("channel = %d\n",buf[0]);

      // Correct signature in non-volatile memory?
      if ((buf[0] == NVOLAT_SIGNATURE_HIGH) && (buf[1] == NVOLAT_SIGNATURE_LOW))
      {
          printf("correct sig\n");
        nvMem.read(buf, DEFAULT_NVOLAT_SECTION, NVOLAT_FREQ_CHANNEL, 2);
        if (buf[0] != 0xFF)
          radio.channel = buf[0];
    
        nvMem.read(buf, DEFAULT_NVOLAT_SECTION, NVOLAT_SYNC_WORD, 2);
        ...
        nvMem.read(devAddress, DEFAULT_NVOLAT_SECTION, NVOLAT_DEVICE_ADDR, 1);
